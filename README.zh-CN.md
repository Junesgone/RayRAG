...
# 2) 编译（只编译 PostgreSQL 后端，不链接 zvec native library）
...
想启用 zvec 原生向量后端，加 `--features postgres-backend,zvec-backend`，
并设置 `RAYRAG_VECTOR_BACKEND=zvec`、`RAYRAG_ZVEC_DIR=/var/lib/rayrag/zvec`、
`ZVEC_LIB_DIR=/zvec/构建产物/lib`。
