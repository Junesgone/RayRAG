...
pub struct AgentChatRequest {
    /// Session to continue. Upstream's agent chat surfaces post `session_id`
    /// (`pages/agent/chat/use-send-agent-message.ts` sets `params.session_id`),
    /// while the earlier RayRAG callers used `conversation_id`; both spellings
    /// resolve to the same stored session.
    #[serde(alias = "session_id")]
    pub conversation_id: Option<String>,
    /// Upstream posts the question as `query`; the form-message path
    /// (`sendFormMessage`) sends only `inputs`, so an absent question is legal.
...
/// POST /api/v1/agents/chat/completions — `api.ts::agentChatCompletion`, the
/// endpoint every upstream agent chat surface posts to
/// (`pages/agent/chat/use-send-agent-message.ts`, reused by the Explore page and
/// the shared-agent embed). The canvas id travels in the body as `agent_id`
/// instead of the path, so the request is forwarded to the same handler as
/// `/api/v1/agents/{id}/completions`; `session_id` selects the session.
#[derive(Deserialize)]
pub struct AgentChatCompletionRequest {
    pub agent_id: String,
    #[serde(flatten)]
    pub chat: AgentChatRequest,
}

pub async fn agent_chat_completion(
    State(state): State<Arc<AppState>>,
    Extension(auth): Extension<AuthContext>,
    Json(body): Json<AgentChatCompletionRequest>,
) -> axum::response::Response {
...
async fn agent_complete_inner(
    ...
    // RAGFlow stores one trace per assistant message under
    // `f"{agent_id}-{message_id}-logs"`, written while the run streams so the log
    // sheet can poll it (`useFetchMessageTrace`, `refetchInterval: 3000`). The
    // collector observes the same lifecycle events the SSE adapter forwards and
    // keeps them in `state.agent_traces`.
    let trace_collector = crate::api::agent_trace::AgentTraceCollector::new(
        state.agent_traces.clone(),
        agent.id.clone(),
        stream.map(|stream| stream.message_id.clone()),
        event_emitter
            .as_ref()
            .map(|emitter| emitter as &dyn crate::agent::WorkflowEventObserver),
    );
    let mut workflow_outcome = if let Some(workflow) = workflow.as_ref() {
        ...
        let outcome = if let Some(claim) = checkpoint_claim.as_ref() {
            ...
            workflow
                .run_interactive_observed(llm, input, &trace_collector)
                .await
        };
        match outcome {
    ...
    if matches!(
        ...
        // RAGFlow persists the run trace under the assistant message id
        // (`{agent_id}-{message_id}-logs`); streaming runs already wrote it while
        // the run was in flight, the non-streaming path publishes it here.
        if let Err(error) = trace_collector.finish(&message_id) {
    ...
    // A paused (UserFillUp) run keeps the trace collected so far, exactly like
    // upstream's incrementally written log key.
    if let Err(error) = trace_collector.finish(&message_id) {
        tracing::warn!(%error, "Failed to persist agent trace");
    }
    if let Some(claim) = checkpoint_claim.as_ref()
...
/// `bot_api.py::begin_inputs` — `GET /api/v1/agentbots/{agent_id}/inputs`.
///
/// The embedded/share surface (`pages/agent/share/index.tsx` through
/// `useFetchExternalAgentInputs`) reads the canvas title, avatar, Begin form,
/// prologue and dialogue mode from here; the caller authenticates with the
/// tenant's `APIToken.beta` secret. Upstream answers
/// `f"Can't find agent by ID: {agent_id}"` when the tenant cannot reach the
/// canvas.
pub async fn agent_bot_inputs(
    State(state): State<Arc<AppState>>,
    Extension(auth): Extension<AuthContext>,
    Path(agent_id): Path<String>,
) -> axum::response::Response {
    let Some(agent) = state.agents.get_accessible(
        ...
        return Json(serde_json::json!({
            "code": 102,
            "message": format!("Can't find agent by ID: {agent_id}"),
            "data": null,
        }))
        .into_response();
    };
    // `canvas.get_component_input_form("begin")`; a canvas without a Begin form
    // still answers with an empty object instead of failing the embed.
    let inputs = crate::agent::agent_component_input_form(&agent.dsl, "begin")
        .map(|form| form.into_owned())
        .unwrap_or_default();
    let prologue = crate::agent::extract_prologue(&agent.dsl).unwrap_or_default();
    let mode = crate::agent::extract_mode(&agent.dsl).unwrap_or_default();
    Json(serde_json::json!({
        "code": 0,
        "message": "success",
        "data": {
...
/// `bot_api.py::agent_bot_completions` —
/// `POST /api/v1/agentbots/{agent_id}/completions`.
///
/// Upstream runs the same `agent.completion(tenant_id, agent_id, **req)`
/// generator as `/agents/<id>/completions`; only the access gate and the SSE
/// envelope differ, so RayRAG reuses the agent chat handler and keeps the
/// upstream `Can't find agent by ID` rejection in front of it.
pub async fn agent_bot_completions(
    State(state): State<Arc<AppState>>,
    Extension(auth): Extension<AuthContext>,
    Path(agent_id): Path<String>,
    Json(body): Json<AgentChatRequest>,
    ...
    agent_complete(State(state), Extension(auth), Path(agent_id), Json(body)).await
}
