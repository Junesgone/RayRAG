...
pub async fn create_data_source(
    ...
    if let Some(config) = body.config {
        ...
        return Err((
            StatusCode::BAD_REQUEST,
            "name/url/target are required".into(),
        ));
    }
    store
        .insert(record.clone())
    ...
    let requested_status = match body.status.as_ref() {
        Some(value) => Some(
            normalize_status(value)
                .ok_or((StatusCode::BAD_REQUEST, "invalid status".to_string()))?,
        ),
        None => None,
    };
    let should_resume =
...
pub async fn get_data_source_logs(
    ...
    Ok(Json(serde_json::json!({ "code": 0, "data": { "total": total, "logs": logs } })))
}
...
async fn perform_sync(
    ...
    let batch = match connector_result {
        ...
        Err(error) => {
            ...
            let kb_name = kb_id
                .and_then(|kb| state.kbs.get(kb).map(|kb| kb.name.clone()));
